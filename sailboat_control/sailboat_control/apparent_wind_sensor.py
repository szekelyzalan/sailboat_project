...
class ApparentWindSensor(Node):
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(
            Odometry, '/vrx/debug/trajectory_body', self.odom_callback, 10) 
    ...
    def odom_callback(self, msg):
        self.boat_vel_x = msg.twist.twist.linear.x
        self.boat_vel_y = msg.twist.twist.linear.y

    def compute_apparent_wind(self):
        # Throttled status log to show us what's happening under the hood
        if self.true_wind_dir is None or self.true_wind_speed is None or self.boat_heading is None:
            self.get_logger().info(
                f"Waiting for data... Wind Dir: {'OK' if self.true_wind_dir is not None else 'MISSING'} | "
                f"Wind Speed: {'OK' if self.true_wind_speed is not None else 'MISSING'} | "
                f"IMU Heading: {'OK' if self.boat_heading is not None else 'MISSING'}",
